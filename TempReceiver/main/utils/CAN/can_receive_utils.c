...
#define CAN_RECEIVE_TASK_TIMEOUT_MS 1000 // Timeout for twai_receive in milliseconds
...
void can_receive_task(void *pvParameters) {
    ...
    while (1) {
        ...
        esp_err_t ret = twai_receive(&rx_message, pdMS_TO_TICKS(CAN_RECEIVE_TASK_TIMEOUT_MS)); // Use defined timeout
        ...
        if (ret == ESP_OK) {
            ...
            // Timeout, no message received within CAN_RECEIVE_TASK_TIMEOUT_MS
            ESP_LOGI(TAG_CAN_RX, "No CAN message received. Waiting...");
